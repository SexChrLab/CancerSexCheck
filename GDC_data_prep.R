...
## Pull out clinical data of interest into lists (for now; could streamline later when I know what I'm planning to do with it)
mf_list <- clindat[["demographic"]][["gender"]]
status_list <- clindat[["demographic"]][["vital_status"]] # This is required, but other follow-up & survival data is optional to include
surv_times  <- clindat[["demographic"]][["days_to_death"]]
follow_list <- clindat[["diagnoses"]][["days_to_last_follow_up"]] # May want to censor the NAs in this list if also alive (i.e., "alive" status may be meaningless)

meta_df <- data.frame(cbind(case_ids,mf_list,status_list,surv_times,follow_list))

# tcga_meta_ids <- UUIDtoBarcode(case_ids, from_type = "case_id")
# meta_df <- data.frame(cbind(id=tcga_meta_ids[1,2],mf_list,status_list,surv_times,follow_list))
...
# fnames <- lapply(ge_manifest$id[1:100], gdcdata) #can download just a subset to test
...
# uuid = substr(files[1],56,91) # uuid from folder name  
